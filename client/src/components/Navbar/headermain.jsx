  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (response.ok) {
           const d = updatedData.reduce((total, item) => total + item.quantity, 0);
           setCartda(d)
        } else {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const data = await response.json();
        const userCartIte = data.whishlist.filter(item => item.userIds === user._id);
        // setWishlistItems(userCartItems);

        // setIsLoading(false);
        if (response.ok) {
          const j = userCartIte.reduce((total, item) => total + item.quantity, 0);
          setWishda(j)
       } else {
         const errorMessage = await response.text();
         console.error(
